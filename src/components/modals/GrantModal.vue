    ...
    <div class="modal__backdrop" style="z-index: 1000">
      ...
      >
        ...
        <div class="modal__body input--transfer">
          Query Grants to
          <input v-model="qgrantee" type="text" />
          <button class="btn--default" @click="getGrants">Get</button>
          <div align="left">
            <div v-for="(gt, i) in grants" :key="gt" class="grantBox">
              <div class="clickBox" @click="show_extra[i] = !show_extra[i]">
                Grant
          ...
          <select v-model="grant">
            <option v-for="(option, i) in options" :key="i" :value="option[1]">
              {{ option[0] }}
            </option>
          </select>
          to
          <input v-model="grantee" type="text" />
          <button class="btn--default" @click="send">Send</button>
          <a v-show="warningText" class="warning"> <br />{{ warningText }} </a>
        </div>
      </div>
    </div>
  </transition>
...
export default {
  ...
  methods: {
    ...
    getGrants() {
      ...
      this.queryGranteeGrants(this.qgrantee).then((grants) => {
