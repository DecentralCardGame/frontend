...
const baseNum: object = { num: 0 };

export const Num = {
  encode(message: Num, writer: Writer = Writer.create()): Writer {
    if (message.num !== 0) {
  ...
  decode(input: Reader | Uint8Array, length?: number): Num {
    const reader = input instanceof Uint8Array ? new Reader(input) : input;
    let end = length === undefined ? reader.len : reader.pos + length;
    const message = { ...baseNum } as Num;
  ...
  fromPartial(object: DeepPartial<Num>): Num {
    const message = { ...baseNum } as Num;
    if (object.num !== undefined && object.num !== null) {
